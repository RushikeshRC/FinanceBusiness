...
class WorkflowChangeDescription {
    constructor(assetsService, editorService, overlayService, plmbrGroupsResource, localizationService) {
        ...
        assetsService.loadJs('/umbraco/lib/jsdiff/diff.min.js').then(() => this.loaded = true);
        localizationService.localizeMany(['workflow_showDiff', 'workflow_viewAttachment', 'workflow_showDiff'])
            .then((resp) => {
            this.showDiffStr = resp[0];
            this.viewAttachmentBtnStr = resp[1];
            this.showDiffBtnStr = resp[2];
        });
    }
    $onInit() {
...
class WorkflowDiff {
    ...
    createDiff() {
        ...
        if (this.currentVersion.name !== this.workflowVersion.name) {
            this.diffs.push({
                label: 'Name',
                diff: JsDiff.diffWords(this.currentVersion.name || '', this.workflowVersion.name || ''),
                isObject: false
            });
        }
        ...
        this.workflowVersion.tabs.forEach((tab, tabIndex) => {
            tab.properties.forEach((workflowProperty, propertyIndex) => {
                ...
                const diff = JsDiff.diffWords(currentProperty.value, workflowProperty.value);
                this.diffs.push({
...
class PlumberHub {
    constructor($rootScope, $q, assetsService) {
        ...
        this.starting = false;
        this.callbacks = [];
        this.platform = Umbraco.Sys.ServerVariables.Plumber.platform;
                ...
                hub = {
                    ...
                    start: () => {
                        ...
                        if (this.platform === 'CORE') {
                            ...
                            if (((_a = $.connection.hub) === null || _a === void 0 ? void 0 : _a.state) !== $.connection.connectionState.connected) {
                                (_b = $.connection.hub) === null || _b === void 0 ? void 0 : _b.start();
                            }
                        }
                    },
...
class OfflineController {
    constructor($rootScope, $timeout, plmbrStateFactory, plmbrWorkflowResource, plmbrActionsService) {
        ...
        if (segments && segments.length === 7) {
            ...
            this.pageUrl = `/${this.nodeId}${Umbraco.Sys.ServerVariables.Plumber.platform === 'CORE' ? '' : '.aspx'}?culture=${this.culture === '*' ? '' : culture}`;
            if (!this.invalid) {
                this.getState();
            }
        }
